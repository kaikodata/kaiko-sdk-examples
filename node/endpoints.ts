...
const compositeIndicesRequest = (creds: grpc.CallCredentials): void => {
    const client = new StreamCompositeIndicesServiceV1Client('gateway-v0-grpc.kaiko.ovh:443', creds as any);
    const request = new StreamCompositeIndicesServiceRequestV1();
    ...
    let count = 0;
    call.on('data', (response: StreamCompositeIndicesServiceResponseV1) => {
        console.log(`[COMPOSITE INDICE] value: ${JSON.stringify(response.toObject())}}`);
    ...
    call.on('end', () => {
        console.log('[COMPOSITE INDICE] Stream ended')
    });
